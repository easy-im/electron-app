...
const FormLable: React.FC<{ name: string; children?: React.ReactNode }> = ({
  name = "",
  children,
}) => {
  return (
...
const RegisterPage: React.FC<Iprops> = ({ toLogin }) => {
  ...
  const onChange = ({
    ...
    e: any;
    type: "mobile" | "password" | "nickname" | "password2";
  }) => {
    setAccounts((pre) => ({
      ...pre,
      [type]: e.target.value,
  ...
  const onRegister = (e: any) => {
    ...
    if (!isPhoneNumber(accounts.mobile)) {
      ...
      !accounts.password ||
      accounts.password.length < 6 ||
      accounts.password.length > 18
    ) {
      setData((pre) => ({
        ...pre,
  ...
  return (
    <div className="registerPage-wrapper">
      <BackBtn cusStyle={{ style: btnWrap }} onClick={toLogin} />
      <div className="title">欢迎使用 Easy IM</div>
